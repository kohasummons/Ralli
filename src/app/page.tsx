...
export interface LetterItem {
  id: string
  type: 'photo' | 'note' | 'voice' | 'spotify' | 'doodle' | 'agent'
  content: string | Blob
...
export default function DigitalLetterComposer() {
  ...
  const [isRobotPolaroidOpen, setIsRobotPolaroidOpen] = useState(false)
      ...
      <div className="h-screen overflow-hidden bg-stone-200 flex flex-col relative">
        ...
        <div className="absolute bottom-0 left-0 right-0 z-30">
          <Toolbar
            ...
            onAgentStart={() => setIsRobotPolaroidOpen(true)}
          />
        </div>
        {isPhotoUploaderOpen && (
          <PhotoUploader
        ...
        )}
        {
          isRobotPolaroidOpen && (
            <AgentConsole
              onClose={() => setIsRobotPolaroidOpen(false)}
              onAgentStart={() => {
                if (!isLoggedIn) {
                  return;
                }
